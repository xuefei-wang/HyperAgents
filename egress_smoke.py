#!/usr/bin/env python3
"""Egress-isolation smoke test for the HyperAgents meta-agent container.
...
REQUIRES a live Docker daemon AND the ``hyperagents`` image already built.
Run:

    python egress_smoke.py

Exit 0 = isolation working (github blocked, provider reachable). Set
KCSI_HA_EGRESS_OPEN=1 to observe the legacy open behaviour for contrast.
"""
import sys
import uuid
...
# NOT installed in slim/CUDA bases. urllib honors the HTTPS_PROXY that
...
def main():
    ...
    if infra is None:
        print("KCSI_HA_EGRESS_OPEN is set: egress isolation DISABLED (open mode).")

    kwargs = dict(image=IMAGE, name=f"hyperagents-egress-smoke-{run_id}", detach=True,
