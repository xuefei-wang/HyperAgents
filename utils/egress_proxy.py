...
``ensureEgressInfra`` and ``egress_allowlist.ts``): the meta-agent container is
attached to a Docker ``--internal`` network with NO route to the internet, so
its only egress path is via this sidecar. The sidecar permits ``CONNECT``
tunnels ONLY to allowlisted hostnames (the configured LLM provider API plus
PyPI); every other host is answered ``403 Forbidden``.

This closes the arbitrary-host egress channel (``git clone github.com`` /
``curl exercism`` / raw sockets) that the meta-agent (unrestricted bash, task =
"modify the codebase") could otherwise use to re-fetch public hidden tests,
while preserving the provider API access the model calls need.
