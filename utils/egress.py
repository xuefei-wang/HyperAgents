"""Network egress isolation for the HyperAgents meta-agent container.

Design mirrors KCSI's egress isolation (parent repo
``runtime_runner/src/container_args.ts`` ``ensureEgressInfra`` +
``egress_allowlist.ts``): the meta-agent container is attached to a Docker
``--internal`` network with no route to the internet; its only egress is an
allowlisting HTTP CONNECT proxy sidecar (``egress_proxy.py``) that permits only
the configured provider API (and PyPI). This closes the arbitrary-host channel
the meta-agent (``tools_available='all'``, task = "modify the codebase") could
use to re-fetch public hidden tests (e.g. cloning Exercism/GitHub).

Removing ``network_mode="host"`` is required: host networking gives the
container the host's full network stack and is fundamentally incompatible with
per-container egress rules. Nothing in the meta-agent path needs host-localhost
services at runtime (the ``fwdproxy:8080`` references are Meta-internal
build-time only, commented out at runtime in utils/gl_utils.py); a local model
endpoint, if any, can be re-allowed via ``KCSI_HA_EGRESS_ALLOW`` and reached
through the proxy.

Default is ISOLATED. Set ``KCSI_HA_EGRESS_OPEN`` truthy (1/true/yes/on) to
restore the legacy host-networking behaviour for debugging. Extra allowlist
hosts: ``KCSI_HA_EGRESS_ALLOW=host1,host2``.

The pure helpers are unit-tested without a Docker daemon; the lifecycle
...
# PyPI is allowlisted because the meta-agent (unrestricted bash) may pip-install
# during code modification; it is not a hidden-test fetch channel. github.com /
# exercism and all other hosts stay blocked.
_PYPI_HOSTS = ["pypi.org", "files.pythonhosted.org"]


def egress_open(env=None) -> bool:
    env = os.environ if env is None else env
    return str(env.get("KCSI_HA_EGRESS_OPEN", "")).strip().lower() in _TRUTHY
...
def derive_allowlist(env=None) -> list:
    ...
    for raw in str(env.get("KCSI_HA_EGRESS_ALLOW", "")).split(","):
...
def agent_run_kwargs(infra) -> dict:
    ...
    open mode (``infra is None``) so the caller keeps legacy host networking."""
    if infra is None:
        return {}
    return {
...
def ensure_egress_infra(client, image_name: str, run_id: str, env=None):
    ...
    infra = EgressInfra(
        internal_net=f"ha-egress-int-{run_id}",
        external_net=f"ha-egress-ext-{run_id}",
        proxy_name=f"ha-egress-proxy-{run_id}",
...
def isolated_run_kwargs(base_kwargs: dict, infra) -> dict:
    ...
    ``KCSI_HA_EGRESS_OPEN``), return ``base_kwargs`` unchanged so the caller keeps
    its legacy networking. ``network_mode`` and ``network`` are mutually exclusive
    in the docker SDK, so the former is popped when isolating.
    """
